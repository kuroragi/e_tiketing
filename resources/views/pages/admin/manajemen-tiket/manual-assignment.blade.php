...
<div class="alert alert-primary border-0 mb-4" role="alert">
    <div class="d-flex">
        ...
        <div>
            ...
            <p class="mb-2">Admin dan pihak berwenang dapat secara fleksibel memilih petugas yang akan menangani tiket berdasarkan:</p>
            <ul class="mb-0">
                <li><strong>Keputusan Pimpinan:</strong> Prioritas sesuai keputusan manajemen</li>
                <li><strong>Keahlian Khusus:</strong> Pilih petugas dengan keahlian spesifik untuk pekerjaan kompleks</li>
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                ...
                                <div class="mb-2">
                                    ...
                                    <span class="badge bg-{{ $ticket['prioritas'] === 'Urgent' ? 'danger' : 'warning' }}">{{ $ticket['prioritas'] }}</span>
                                    <span class="badge bg-secondary">{{ $ticket['tanggal_masuk']->diffForHumans() }}</span>
                                </div>
                                <small class="text-muted">{{ $ticket['deskripsi'] }}</small>
                            </div>
                            <div class="col-md-4">
                                <button type="button" class="btn btn-primary w-100" data-bs-toggle="modal" data-bs-target="#assignModal{{ $ticket['id'] }}">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-4">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            <div class="card cursor-pointer petugas-card" data-petugas="{{ $petugas['id'] }}" onclick="selectPetugas(this, {{ $petugas['id'] }}, '{{ $ticket['id'] }}')">
                                                <div class="card-body">
                                                    <div class="form-check">
                                                        <input class="form-check-input petugas-radio" type="radio" name="petugas_{{ $ticket['id'] }}" value="{{ $petugas['id'] }}" id="petugas_{{ $ticket['id'] }}_{{ $petugas['id'] }}">
                                                        <label class="form-check-label w-100" for="petugas_{{ $ticket['id'] }}_{{ $petugas['id'] }}">
                                                            <h6 class="mb-1">{{ $petugas['nama'] }}</h6>
                                                            <small class="text-muted d-block mb-2">{{ $petugas['skill'] }}</small>
                                                            <span class="badge bg-warning">Load: {{ $petugas['load'] }} tiket</span>
                                                        </label>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div class="alert alert-info" role="alert">
                                    ...
                                    <small>
                                        Pastikan petugas yang dipilih memiliki keahlian yang sesuai dengan jenis pekerjaan. 
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary" onclick="confirmAssignment('{{ $ticket['id'] }}')">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    <div class="d-flex justify-content-between align-items-start mb-2">
                        ...
                        <span class="badge bg-{{ $petugas['load'] === 0 ? 'success' : ($petugas['load'] <= 2 ? 'info' : 'warning') }}">
                            {{ $petugas['load'] }} tiket
                        </span>
                    </div>
                    <small class="text-muted d-block mb-2">{{ $petugas['skill'] }}</small>
...
function selectPetugas(element, petugasId, ticketId) {
    ...
    radios.forEach(radio => {
        radio.parentElement.parentElement.parentElement.classList.remove('border-primary', 'bg-primary-light');
    });
...
function confirmAssignment(ticketId) {
    ...
    fetch(`/api/ticket/manual-assign/${ticketId}`, {
        ...
        body: JSON.stringify({ petugas_id: selectedPetugas.value })
    })
